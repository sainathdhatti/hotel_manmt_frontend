...
const UserDashboard = () => {
  ...
  const { user, getAllUsers, updateUser, loading, error, deleteUser } = useUserStore((state) => ({
  ...
  const handleUpdateUser = async () => {
    if (editingUser && userId !== null) {
      try {
        await updateUser(userId, editingUser); // Ensure userId and editingUser are sent correctly
              ...
              <tbody>
                {filteredOrders.map((order) => (
                  <tr key={order.id} className="hover:bg-gray-50">
                    <td className="py-3 px-4 border-b">{order.user ? order.user.firstName : "Unknown"}</td>
                    <td className="py-3 px-4 border-b">{new Date(order.order_time).toLocaleString()}</td>
