...
import useBookingsStore, { Booking, BookingStatuss } from "@/app/store/bookingStore";
...
const Receptionist = () => {
  ...
  const { bookings, fetchBookings, updateBookingStatus } = useBookingsStore((state) => ({
    bookings: state.bookings,
    fetchBookings: state.fetchBookings,
    updateBookingStatus: state.updateBookingStatus,
  }));

  const { finalbillings, fetchAllBillings } = useFinalBillingStore((state) => ({
    finalbillings: state.finalBillings,
    fetchAllBillings: state.getFinalBillings,
  }));

  const { calculateFinalBillings } = useFinalBillingStore((state) => ({
    calculateFinalBillings: state.calculateFinalBillings,
  }));
  ...
  const handleStatusChange = async (bookingId: number, status: BookingStatuss) => {
    try {
    ...
    switch (currentView) {
        ...
        return (
          <div className="flex flex-col items-center justify-center">
            <div className="w-full border-2 border-gray-200 p-6">
              <h1 className="text-2xl font-bold mb-4 text-teal-500">Bookings List</h1>
                        ...
                        <td className="px-4 py-2 text-center text-pink-500">
                          <select
                            ...
                            onChange={(e) => {
                              ...
                              if (booking.status !== BookingStatuss.CHECKED_OUT && booking.status !== BookingStatuss.CANCELLED) {
                                handleStatusChange(booking.bookingId, newStatus);
                              }
                            }}
                            className={`border border-gray-300 rounded-md p-1 ${
                              booking.status === BookingStatuss.CHECKED_OUT ? "bg-gray-200 cursor-not-allowed" : ""
                            }`}
                            disabled={booking.status === BookingStatuss.CHECKED_OUT || booking.status === BookingStatuss.CANCELLED}
                          >
      ...
      case "billings":
        return (
          <div className="flex flex-col items-center justify-center">
            <div className="w-full border-2 border-gray-200 p-6">
              <h1 className="text-2xl font-bold mb-4 text-teal-500">Billing List</h1>
              <table className="min-w-full bg-white border border-gray-300">
                <thead className="bg-gray-100 text-teal-600">
                  <tr>
      ...
      default:
        return (
          <div className="text-center text-gray-500">Select a view from the sidebar.</div>
        );
    }
  };
